describe("<book-cell> directive ", function () {
	var injector;
	var $element;
	var $scope;
	var template;
	...
	beforeEach(function () {
		injector = angular.injector(["ng","ngMock", "bookStore", "wwwroot/template/bookCellDirective.html"]);
		
		injector.invoke(function ($templateCache, $httpBackend, $rootScope, $compile) {
	...
	it("should display cover and title of the book", function () {
		...
		expect($element.find("img").attr("src")).toEqual(book.pictures[0]);
	});
})